...
export default function Home() {
  ...
  useEffect(() => {
    const checkRole = async () => {
      if (!isLoggedIn) return;
      try {
        const res = await axios.get("https://viadocs-backend-u977.onrender.com/api/profile", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!res.data.role) {
          // open role modal
  ...
  const saveRole = async (role) => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://viadocs-backend-u977.onrender.com/api/profile/role",
        { role },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const setFavorite = async (id) => {
    try {
      const res = await axios.patch(
        `https://viadocs-backend-u977.onrender.com/api/docs/my-docs/${id}/favorite`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setDocs((prev) =>
  ...
  const shareDocAsPDF = async (doc) => {
    try {
      const res = await axios.get(`https://viadocs-backend-u977.onrender.com/api/docs/my-docs/${doc._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`https://viadocs-backend-u977.onrender.com/api/docs/my-docs/${deleteTarget._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
